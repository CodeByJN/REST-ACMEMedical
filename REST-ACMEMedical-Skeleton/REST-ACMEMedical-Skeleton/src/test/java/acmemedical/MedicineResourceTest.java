...
public class MedicineResourceTest {
    ...
    static final String BASE_URI = "http://localhost:8080/rest-acmemedical/api/v1/medicines";
    ...
    @Test
    void userCannotCreateMedicine() {
        String json = "{\"name\":\"FakeMed\",\"dosage\":\"1mg\"}";
        Response resp = authRequest(BASE_URI, normalUser, normalPass).post(Entity.json(json));
        assertEquals(403, resp.getStatus());
    }

    @Test
    void adminCanGetMedicineById() {
        Response resp = authRequest(BASE_URI + "/1", adminUser, adminPass).get();
        assertTrue(resp.getStatus() == 200 || resp.getStatus() == 404);
    }

    @Test
    void userCanGetMedicineById() {
        Response resp = authRequest(BASE_URI + "/1", normalUser, normalPass).get();
        assertTrue(resp.getStatus() == 200 || resp.getStatus() == 404);
    }

    @Test
    void adminCanDeleteMedicine() {
